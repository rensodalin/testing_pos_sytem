...
const BottomNav = () => {
  ...
  const handleCreateOrder = async () => {
    ...
    try {
      // ✅ Send order to backend with correctly parsed integers
      const res = await createOrder({
        customer: name,
        phone,
        guests: parseInt(guestCount),
        tableId: parseInt(selectedTableData.id),
        tableNo: selectedTableData.name,
      });
  
      // Dispatch to Redux (optional)
      dispatch(setCustomer({ name, phone, guests: guestCount }));
      dispatch(updateTable({ table: { tableId: selectedTableData.id, tableNo: selectedTableData.name } }));
